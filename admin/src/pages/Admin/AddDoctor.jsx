...
const AddDoctor = () => {
    ...
    <p className='mb-3 text-lg font-semibold text-white'>Add Doctor</p>

    <div className='bg-gray-900 px-8 py-8 border border-gray-700 rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        <div className='flex items-center gap-4 mb-8 text-gray-300'>
            <label htmlFor="doc-img">
                <img className='w-16 h-16 object-cover bg-gray-700 rounded-full cursor-pointer' src={docImg ? URL.createObjectURL(docImg) : assets.upload_area} alt="Upload" />
            </label>
            <input onChange={(e) => setDocImg(e.target.files[0])} type="file" id="doc-img" hidden />
            <p className='text-sm'>Upload doctor <br /> picture</p>
        </div>

        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-300'>
            ...
            <div className='w-full lg:flex-1 flex flex-col gap-4'>
                ...
                <div className='flex-1 flex flex-col gap-1'>
                    ...
                    <select onChange={e => setExperience(e.target.value)} value={experience}
                        className='bg-gray-800 text-white border border-gray-700 rounded px-2 py-2'>
        ...
        <div>
            <p className='mt-4 mb-2 text-white'>About Doctor</p>
            <textarea onChange={e => setAbout(e.target.value)} value={about}
                className='w-full bg-gray-800 text-white px-4 pt-2 border border-gray-700 rounded focus:outline-none focus:ring-2 focus:ring-primary'
                rows={5} placeholder='Write about doctor'></textarea>
        </div>

        <button type='submit'
            className='bg-primary hover:bg-primary-dark transition-colors px-10 py-3 mt-4 text-white font-semibold rounded-full'>
            Add doctor
        </button>
    </div>
